...
def test_status_block_says_the_state_the_days_and_the_rules():
    ...
    assert ("Status Easing A Worse stretch ended within the last 3 months, after no light had been on for 3 trading days in a row, and none is on now. "
            "The market can still be well below its high.") in r and "Status Recovering" not in r
...
def test_status_block_is_left_out_or_partial_when_data_is_missing_and_never_breaks_the_card():
    ...
    assert "Status Calm" in none and "Since 1991 there have been" not in none and "A hypothetical" not in none and "Every Worse stretch since 1991" in none
